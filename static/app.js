...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- EXECUTIVE DATA EXPORTER & REPORT BUILDER CONTROLLER ---
    function initDataExporterModal() {
        ...
        // DOM Elements
        const btnOpenHeader = document.getElementById('btnOpenExportModal');
        const btnOpenDrawer = document.getElementById('btnMobileDrawerExportReports');
        const btnClose = document.getElementById('btnCloseExportModal');
        const btnCloseFooter = document.getElementById('btnCloseExportModalFooter');

        const datasetTabs = document.querySelectorAll('.export-ds-tab');
        const presetBtns = document.querySelectorAll('.export-preset-pill');
        const btnResetFilters = document.getElementById('btnExportResetFilters');

        const fltDistrict = document.getElementById('exportFltDistrict');
        const fltDesignation = document.getElementById('exportFltDesignation');
        const fltOccupancy = document.getElementById('exportFltOccupancy');
        const fltTenure = document.getElementById('exportFltTenure');
        const fltSuperannuation = document.getElementById('exportFltSuperannuation');
        const fltQuota = document.getElementById('exportFltQuota');
        const fltSearch = document.getElementById('exportFltSearch');

        const contOccupancy = document.getElementById('exportFltOccupancyContainer');
        const contTenure = document.getElementById('exportFltTenureContainer');
        const contSuperannuation = document.getElementById('exportFltSuperannuationContainer');
        const contQuota = document.getElementById('exportFltQuotaContainer');

        const sortBySelect = document.getElementById('exportSortBy');
        const btnSortAsc = document.getElementById('btnExportSortAsc');
        const btnSortDesc = document.getElementById('btnExportSortDesc');

        const columnsContainer = document.getElementById('exportColumnsContainer');
        const btnColsSelectAll = document.getElementById('btnExportColsSelectAll');
        const btnColsResetDefault = document.getElementById('btnExportColsResetDefault');

        const previewCount = document.getElementById('exportPreviewCount');
        const previewFiltersSummary = document.getElementById('exportPreviewFiltersSummary');
        const previewTableHead = document.getElementById('exportPreviewTableHead');
        const previewTableBody = document.getElementById('exportPreviewTableBody');

        const btnDownloadExcel = document.getElementById('btnDownloadExportExcel');
        const btnDownloadDocx = document.getElementById('btnDownloadExportDocx');

        // Fetch Metadata
        async function fetchMeta() {
            if (exportMeta) return exportMeta;
            try {
                const res = await fetch('/api/export/meta');
                if (!res.ok) throw new Error('Failed to load export metadata');
                exportMeta = await res.json();

                // Populate District dropdown
                if (fltDistrict && exportMeta.filters && exportMeta.filters.districts) {
                    fltDistrict.innerHTML = '<option value="">All Districts (Statewide)</option>' +
                        exportMeta.filters.districts.map(d => `<option value="${d}">${d}</option>`).join('');
                }

                // Populate Designation dropdown
                if (fltDesignation && exportMeta.filters && exportMeta.filters.designations) {
                    fltDesignation.innerHTML = '<option value="">All Designations</option>' +
                        exportMeta.filters.designations.map(d => `<option value="${d}">${d}</option>`).join('');
                }

                return exportMeta;
            } catch (err) {
                console.error('Error fetching export meta:', err);
                return null;
            }
        }

        // Configure view for active dataset
        function setupDatasetView(datasetKey) {
            currentExport.dataset = datasetKey;
            ...
            if (contOccupancy) {
                const showOccupancy = (datasetKey === 'cadre_posts' || datasetKey === 'posts_with_employees' || datasetKey === 'obliterated');
                contOccupancy.style.display = showOccupancy ? '' : 'none';
                if (!showOccupancy) {
                    currentExport.filters.occupancy_status = '';
                    if (fltOccupancy) fltOccupancy.value = '';
                }
            }

            if (contTenure) {
                const showTenure = (datasetKey === 'cadre_posts' || datasetKey === 'posts_with_employees' || datasetKey === 'obliterated');
                contTenure.style.display = showTenure ? '' : 'none';
                if (!showTenure) {
                    currentExport.filters.tenure_filter = '';
                    if (fltTenure) fltTenure.value = '';
                }
            }

            if (contSuperannuation) {
                const showSuperann = (datasetKey === 'employees' || datasetKey === 'cadre_posts' || datasetKey === 'posts_with_employees' || datasetKey === 'roster');
                contSuperannuation.style.display = showSuperann ? '' : 'none';
                if (!showSuperann) {
                    currentExport.filters.superannuation_filter = '';
                    if (fltSuperannuation) fltSuperannuation.value = '';
                }
            }

            if (contQuota) {
                const showQuota = (datasetKey === 'roster');
                contQuota.style.display = showQuota ? '' : 'none';
                if (!showQuota) {
                    currentExport.filters.roster_quota = '';
                    if (fltQuota) fltQuota.value = '';
                }
            }

            if (!exportMeta || !exportMeta.datasets || !exportMeta.datasets[datasetKey]) return;
            const dsSchema = exportMeta.datasets[datasetKey];

            // Setup Sort By options
            if (sortBySelect) {
                sortBySelect.innerHTML = dsSchema.columns.map(c =>
                    `<option value="${c.key}">${c.label}</option>`
                ).join('');
                sortBySelect.value = dsSchema.default_sort || dsSchema.columns[0]?.key;
                currentExport.sort_by = sortBySelect.value;
            }

            // Set sort order
            currentExport.sort_order = dsSchema.default_order || 'asc';
            updateSortButtons();

            // Setup selectable columns
            const defaultCols = dsSchema.columns.filter(c => c.is_default).map(c => c.key);
            currentExport.selected_columns = [...defaultCols];
            renderColumnCheckboxes(dsSchema.columns, currentExport.selected_columns);

            fetchPreview();
        }

        function renderColumnCheckboxes(allColumns, selectedKeys) {
            ...
            columnsContainer.querySelectorAll('.export-col-checkbox').forEach(cb => {
                cb.addEventListener('change', () => {
                    currentExport.selected_columns = Array.from(columnsContainer.querySelectorAll('.export-col-checkbox:checked')).map(c => c.value);
                    fetchPreview();
                });
            });
        }
        ...
        const debouncedFetchPreview = debounce(fetchPreview, 250);

        async function fetchPreview() {
            if (!previewCount || !previewTableBody) return;
            previewCount.innerHTML = `<span class="animate-pulse">Calculating matching records...</span>`;

            try {
                // Filter out empty filter values
                const cleanFilters = {};
                for (const [k, v] of Object.entries(currentExport.filters)) {
                    if (v !== '' && v !== null && v !== undefined) cleanFilters[k] = v;
                }

                const payload = {
                    dataset: currentExport.dataset,
                    filters: cleanFilters,
                    sort_by: currentExport.sort_by,
                    sort_order: currentExport.sort_order,
                    selected_columns: currentExport.selected_columns,
                    limit: 5
                };
                ...
                if (!exportMeta || !exportMeta.datasets || !exportMeta.datasets[currentExport.dataset]) return;
                const dsSchema = exportMeta.datasets[currentExport.dataset];
                const activeCols = (currentExport.selected_columns && currentExport.selected_columns.length > 0)
                    ? dsSchema.columns.filter(c => currentExport.selected_columns.includes(c.key))
                    : dsSchema.columns.filter(c => c.is_default);

                if (previewTableHead) {
                    previewTableHead.innerHTML = `
                        <tr>
                            ${activeCols.map(c => `<th class="px-3 py-2 font-bold uppercase tracking-wider text-[10px] text-neutral-600 bg-neutral-100 border-b border-neutral-200 text-${c.align || 'left'}">${c.label}</th>`).join('')}
                        </tr>
                    `;
                }
                ...
                if (!data.sample || data.sample.length === 0) {
                    previewTableBody.innerHTML = `
                        <tr>
                            <td colspan="${activeCols.length || 1}" class="text-center py-8 text-neutral-400 font-sans">
                                <div class="flex flex-col items-center justify-center gap-1.5">
                                    <i data-lucide="inbox" class="w-5 h-5 text-neutral-300"></i>
                ...
                previewTableBody.innerHTML = data.sample.map((row, idx) => {
                    const bg = idx % 2 === 0 ? 'bg-white' : 'bg-neutral-50/70';
                    return `
                        <tr class="${bg} hover:bg-neutral-100 transition">
                            ${activeCols.map(c => {
                                let val = row[c.key];
                                if (val === null || val === undefined || val === '') val = '—';

                                let cellHtml = String(val);
                                if (c.key === 'occupancy_status') {
                                    if (val === 'Vacant') cellHtml = '<span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-emerald-100 text-emerald-800">Vacant</span>';
                                    else if (val === 'Occupied') cellHtml = '<span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-neutral-200 text-neutral-800">Occupied</span>';
                                } else if (c.key === 'tenure_over_flag') {
                                    if (val === 'Over-Tenure' || val === 'Yes') cellHtml = '<span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-rose-100 text-rose-800">Over-Tenure</span>';
                                    else cellHtml = '<span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-emerald-50 text-emerald-700">Normal</span>';
                                } else if (c.key === 'allotment_status') {
                                    if (val === 'Allotted') cellHtml = '<span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-sky-100 text-sky-800">Allotted</span>';
                                    else cellHtml = '<span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-neutral-200 text-neutral-700">Pending</span>';
                                }
                                return `<td class="px-3 py-2 border-b border-neutral-100 text-${c.align || 'left'} whitespace-nowrap overflow-hidden text-ellipsis max-w-[240px]">${cellHtml}</td>`;
                            }).join('')}
                        </tr>
                    `;
                }).join('');
                ...
                previewCount.innerText = 'Matching Records: Error calculating';
            }
        }

        // Apply Preset
        function applyPreset(presetId) {
            if (!exportMeta || !exportMeta.presets) return;
            const preset = exportMeta.presets.find(p => p.id === presetId);
            if (!preset) return;

            // Clear all current filters first
            currentExport.filters = {
                district: '',
                designation: '',
                occupancy_status: '',
                tenure_filter: '',
            ...
            // Reset UI inputs
            if (fltDistrict) fltDistrict.value = '';
            if (fltDesignation) fltDesignation.value = '';
            if (fltOccupancy) fltOccupancy.value = '';
            if (fltTenure) fltTenure.value = '';
            if (fltSuperannuation) fltSuperannuation.value = '';
            if (fltQuota) fltQuota.value = '';
            ...
            // Apply preset params
            Object.assign(currentExport.filters, preset.params);

            // Update UI inputs from preset params
            if (preset.params.occupancy_status && fltOccupancy) fltOccupancy.value = preset.params.occupancy_status;
            if (preset.params.tenure_filter && fltTenure) fltTenure.value = preset.params.tenure_filter;
            if (preset.params.superannuation_filter && fltSuperannuation) fltSuperannuation.value = preset.params.superannuation_filter;
            if (preset.params.roster_quota && fltQuota) fltQuota.value = preset.params.roster_quota;
            if (preset.params.district && fltDistrict) fltDistrict.value = preset.params.district;

            // Switch to preset dataset
            setupDatasetView(preset.dataset);
            window.showToast(`Applied preset: ${preset.label}`, 'info');
        }
        ...
        function resetAllFilters() {
            ...
            fetchPreview();
            window.showToast('All export filters cleared', 'info');
        }

        // Download Report (Excel or Word)
        async function downloadReport(format) {
            const btn = format === 'excel' ? btnDownloadExcel : btnDownloadDocx;
            const origHtml = btn ? btn.innerHTML : '';
            if (btn) {
                btn.disabled = true;
                btn.innerHTML = `<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i><span>Generating ${format === 'excel' ? 'Excel' : 'Word'}...</span>`;
                if (window.lucide) lucide.createIcons();
            }
            window.showToast(`Generating official West Bengal ARD ${format === 'excel' ? 'Excel (.xlsx)' : 'Word (.docx)'} extract...`, 'info');
            ...
            try {
                ...
                const payload = {
                    ...
                    sort_by: currentExport.sort_by,
                    sort_order: currentExport.sort_order,
                    selected_columns: (currentExport.selected_columns && currentExport.selected_columns.length > 0) ? currentExport.selected_columns : null
                };
                ...
                if (!res.ok) {
                    const errJson = await res.json().catch(() => ({ detail: 'Download failed' }));
                    throw new Error(errJson.detail || 'Failed to generate report');
                }

                const blob = await res.blob();
                let filename = `WB_ARD_${currentExport.dataset}_Report.${format === 'excel' ? 'xlsx' : 'docx'}`;
                ...
                window.showToast(err.message || 'Export error occurred', 'error');
            } finally {
                if (btn) {
                    btn.disabled = false;
        ...
        async function openModal() {
            modal.classList.remove('hidden');
            await fetchMeta();
            setupDatasetView(currentExport.dataset);
            if (window.lucide) lucide.createIcons();
        }
        ...
        window.closeExportModal = closeModal;
        ...
        datasetTabs.forEach(tab => {
            tab.addEventListener('click', () => {
                const ds = tab.getAttribute('data-export-dataset');
                if (ds) setupDatasetView(ds);
            });
        });
        ...
        // Filter Inputs
        fltDistrict?.addEventListener('change', (e) => {
            currentExport.filters.district = e.target.value;
            debouncedFetchPreview();
        });

        fltDesignation?.addEventListener('change', (e) => {
            currentExport.filters.designation = e.target.value;
            debouncedFetchPreview();
        });

        fltOccupancy?.addEventListener('change', (e) => {
            currentExport.filters.occupancy_status = e.target.value;
            debouncedFetchPreview();
        });

        fltTenure?.addEventListener('change', (e) => {
            currentExport.filters.tenure_filter = e.target.value;
            debouncedFetchPreview();
        });

        fltSuperannuation?.addEventListener('change', (e) => {
            currentExport.filters.superannuation_filter = e.target.value;
            debouncedFetchPreview();
        });

        fltQuota?.addEventListener('change', (e) => {
            currentExport.filters.roster_quota = e.target.value;
            debouncedFetchPreview();
        });

        fltSearch?.addEventListener('input', (e) => {
            currentExport.filters.search = e.target.value.trim();
            debouncedFetchPreview();
        });

        // Sorting
        sortBySelect?.addEventListener('change', (e) => {
            currentExport.sort_by = e.target.value;
            debouncedFetchPreview();
        });

        btnSortAsc?.addEventListener('click', () => {
            currentExport.sort_order = 'asc';
            updateSortButtons();
            debouncedFetchPreview();
        });

        btnSortDesc?.addEventListener('click', () => {
            currentExport.sort_order = 'desc';
            updateSortButtons();
            debouncedFetchPreview();
        });
        ...
        btnColsSelectAll?.addEventListener('click', () => {
            if (!exportMeta || !exportMeta.datasets || !exportMeta.datasets[currentExport.dataset]) return;
            const allCols = exportMeta.datasets[currentExport.dataset].columns;
            currentExport.selected_columns = allCols.map(c => c.key);
            renderColumnCheckboxes(allCols, currentExport.selected_columns);
            fetchPreview();
        });
        ...
        btnColsResetDefault?.addEventListener('click', () => {
            if (!exportMeta || !exportMeta.datasets || !exportMeta.datasets[currentExport.dataset]) return;
            const allCols = exportMeta.datasets[currentExport.dataset].columns;
            currentExport.selected_columns = allCols.filter(c => c.is_default).map(c => c.key);
            renderColumnCheckboxes(allCols, currentExport.selected_columns);
            fetchPreview();
        });

        // Download buttons
        btnDownloadExcel?.addEventListener('click', () => downloadReport('excel'));
