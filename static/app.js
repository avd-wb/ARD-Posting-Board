...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ensure footer secret button and analytics shortcut are active immediately
    setTimeout(() => {
        if (window.initVisitorAnalyticsUI) window.initVisitorAnalyticsUI();
    }, 50);
    ...
    function switchTab(targetTab) {
        if (!targetTab) return;
        if (window.sendTelemetryPing) window.sendTelemetryPing('tab_switch', targetTab);
            ...
            tbody.innerHTML = json.data.map(c => {
                ...
                return `
                    <tr class="hover:bg-slate-50 transition ${isRet ? 'bg-slate-100/75 opacity-70' : (isAllotted ? 'bg-emerald-50/20' : '')}">
                        ...
                        <td class="py-2.5 px-2 text-slate-700 font-medium">${c.point_reserved_for || '—'}</td>
                        <td class="py-2.5 px-4">
    ...
    window.openDualAllotModal = async function(hrmsId, source) {
        ...
        if (officer) {
            ...
            subtitle.innerText = `${offName}${officer.point_reserved_for ? ` | Quota: ${officer.point_reserved_for}` : ''}`;
            ...
            rosterPt.innerText = officer.roster_point ? `Point ${officer.roster_point} (${officer.point_reserved_for || 'General'})` : `Cadre Post`;
        ...
        try {
            ...
            container.innerHTML = `
                ...
                <div id="dossierSecProfile" class="dossier-sec space-y-4">
                    ...
                    <!-- Identity & Profile Header with Photo -->
                        ...
                        <div class="flex-1 space-y-2">
                            ...
                            ${d.languages ? `
                                <div class="pt-2 border-t border-slate-200 text-[11px]">
                                    <div><strong class="text-slate-600">Languages:</strong> <span class="text-slate-800 font-medium">${d.languages}</span></div>
                                </div>
                            ` : ''}
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 p-3 rounded-lg bg-blue-50/50 border border-blue-200 text-center">
                        <div>
                        ...
                        <div>
                            ...
                            <div class="text-[9px] text-blue-600 font-medium">WBSR Rule 75(a) Compliant</div>
                        </div>
                    </div>
                ...
                <!-- TAB 4: DISTRICT POSTINGS / INTERACTIVE ALLOCATION COCKPIT -->
...
(function() {
    ...
    // =========================================================================
    // VISITOR TELEMETRY & ACCESS INTELLIGENCE CONTROLLER
    // =========================================================================
    ...
    function initVisitorTelemetry() {
        sendTelemetryPing('pageview', 'Cadre Directory');
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'hidden') {
                ...
                sendTelemetryPing('resume');
            }
        });

        window.addEventListener('beforeunload', () => {
            sendTelemetryPing('unload');
        });
    }
    ...
    async function loadVisitorAnalytics() {
        ...
        try {
                        ...
                        return `
                            <div class="space-y-1">
                                <div class="flex items-center justify-between text-xs">
                                    ...
                                    <span class="font-mono text-slate-500 text-[11px]">${c.count} (${pct}%)</span>
                                </div>
                                <div class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                                    <div class="bg-wbblue-600 h-1.5 rounded-full" style="width: ${pct}%"></div>
                                </div>
                            </div>
                        `;
                    }).join('');
            ...
            // 3. Render Devices Breakdown
            if (devicesList) {
                const devs = stats.devices || {};
                const oss = stats.os_breakdown || {};
                        ...
                        <div class="space-y-2">
                            <div class="grid grid-cols-3 gap-1 text-center">
                                ${Object.entries(devs).map(([k, v]) => `
                                    <div class="p-2 rounded-xl bg-slate-50 border border-slate-200/60">
                                        <div class="font-bold text-slate-800 text-xs">${v}</div>
                                        <div class="text-[10px] text-slate-500">${k}</div>
                                    </div>
                                `).join('')}
                            </div>
                            <div class="pt-1 flex flex-wrap gap-1">
                                ${Object.entries(oss).map(([k, v]) => `
                                    <span class="px-2 py-0.5 rounded-md bg-purple-50 text-purple-700 border border-purple-200 text-[10px] font-medium">
                                        ${k}: ${v}
                                    </span>
                                `).join('')}
                            </div>
    ...
    function initVisitorAnalyticsUI() {
        ...
        if (btnFooterSecret && !btnFooterSecret._bound) {
            btnFooterSecret._bound = true;
            btnFooterSecret.addEventListener('click', (e) => {
                e.stopPropagation();
                openVisitorAnalyticsModal();
            });
        }

        const containerFooterSecret = document.getElementById('containerFooterSecretTraffic');
        if (containerFooterSecret && !containerFooterSecret._bound) {
            containerFooterSecret._bound = true;
            containerFooterSecret.addEventListener('click', openVisitorAnalyticsModal);
        }

        // Discrete admin shortcut: Alt+Shift+T
        if (!window._analyticsKeyBound) {
            window._analyticsKeyBound = true;
            window.addEventListener('keydown', (e) => {
        ...
        if (searchInput && !searchInput._bound) {
            ...
            searchInput.addEventListener('input', () => {
                ...
                searchInput._deb = setTimeout(loadVisitorAnalytics, 300);
            });
        }
    ...
    window.initVisitorAnalyticsUI = initVisitorAnalyticsUI;
});
